...
#SBATCH --ntasks=1
#SBATCH --nodes=1
#SBATCH --array=1-100%10             # 125 jobs, max 10 running at once
...
cd "Experiment_1/" || { echo "ERROR: Directory 'Experiment_1/' not found."; exit 1; }
mkdir -p results_snellius_extra

PARAMS_FILE="params_ex1.txt"
BATCH_SIZE=16
...
for i in $(seq 0 $((BATCH_SIZE - 1))); do
  ...
  read -r group_rounds turnover_rounds pop_size network mode run_id replace<<< "$(echo "$line" | tr -d '\r' | xargs)"
