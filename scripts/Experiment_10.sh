...
#SBATCH --ntasks=1
#SBATCH --nodes=1
#SBATCH --array=1-375%10             # 375 jobs, max 10 running at once
...
cd "alpha_beta/" || { echo "ERROR: Directory 'alpha_beta/' not found."; exit 1; }
mkdir -p results_snellius

PARAMS_FILE="params_alpha_beta.txt"
BATCH_SIZE=16
...
for i in $(seq 0 $((BATCH_SIZE - 1))); do
  ...
  read -r group_rounds turnover_rounds pop_size network mode run_id dynamic replace beta alpha <<< "$(echo "$line" | tr -d '\r' | xargs)"
