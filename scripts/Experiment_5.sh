...
#SBATCH --ntasks=1
#SBATCH --nodes=1
#SBATCH --array=1-1%10             # 125 jobs, max 10 running at once
...
cd "network_structures/" || { echo "ERROR: Directory 'Experiment_1/' not found."; exit 1; }
mkdir -p results_snellius_extra

PARAMS_FILE="missing_ex1_extra.txt"
BATCH_SIZE=16
...
for i in $(seq 0 $((BATCH_SIZE - 1))); do
  ...
  read -r group_rounds turnover_rounds pop_size network mode run_id dynamic replace alpha <<< "$(echo "$line" | tr -d '\r' | xargs)"
  ...
  python sim_extra.py \
    ...
    --replace "$replace"\
    --alpha "$alpha"\
    > "results_snellius_extra/out_${idx}.txt" 2>&1 &
done
