...
cd "weaker_prior/" || { echo "ERROR: Directory 'Experiment_1/' not found."; exit 1; }
mkdir -p results_snellius

PARAMS_FILE="params_beta.txt"
BATCH_SIZE=16
...
for i in $(seq 0 $((BATCH_SIZE - 1))); do
  ...
  read -r group_rounds turnover_rounds pop_size network mode run_id dynamic replace beta <<< "$(echo "$line" | tr -d '\r' | xargs)"
