...
def population_communication(population, population_dict, languages, meanings, signals, 
    ...
    for _ in range(round_with_one_agent):
        ...
        learner.prior = smooth_posterior(learner.prior, compress_prior, alpha)
...
def get_network_data(network):
    ...
    if nx.is_connected(network):
        ...
        network_data['average_shortest_path'] = "NaN"
...
def simulation(generations, languages, meanings, signals, rounds, popsize, turnover_rounds, 
    ...
    network_list.append(get_network_data(network))
            ...
            if turnover_rounds > 0:
                for i in range(extra_agent_to_add + 1): # add at least one new agent, and additionally extra if more were removed previously
                    ...
                    for _ in range(turnover_rounds):
                        ...
                        new_agent.prior = smooth_posterior(new_agent.prior, compress_prior, alpha)
    ...
    data = pd.DataFrame(network_list)
...
def main():
    ...
    priors = [log_non_normed_prior(g) for g in gr_strings]
    ...
    result, data = simulation(
        ...
        priors, r, types, args.network_type, edge_add, 
        edge_remove, args.alpha
    )

    # name = f"h{args.group_round}_v{args.turnover_round}_p{args.pop_size}"
    # results_dir = f"results_snellius_extra/{args.mode}/{args.replace}/{args.alpha}/{name}"
    ...
    results_dir = f"results_snellius/alpha/{args.mode}/{args.network_type}/"
