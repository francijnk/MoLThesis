...
def simulation(generations, languages, meanings, signals, rounds, popsize, turnover_rounds, 
               ...
               network_type, dynamic, edge_add, edge_remove):
    results = []
    population_dict = {}
        ...
        if turnover_rounds > 0:
            ...
            for _ in range(len(population)):
                ...
                if p < replace:
                    ...
                    picked_agent = pick_based_on_communicative_success(population, languages, mode, meanings, signals, error_probability)
                    to_be_removed_id = picked_agent.id
                    new_agent = Agent(id=to_be_removed_id, prior=compress_prior.copy(), neighbors = [n for n in network.neighbors(to_be_removed_id)], age=0)
                    old_population.remove(picked_agent)

                    teacher = np.random.choice(old_population)
                    for _ in range(turnover_rounds):
                            
                        meaning = np.random.choice(meanings)

                        s_language = sample(teacher.prior, languages, mode)
                        signal = pragmatic_speaker(s_language, meaning, meanings, signals, error_probability) # pragmatic signal
...
def main():
    ...
    parser.add_argument("--replace", type=float, required=True)
    ...
    result, data = simulation(
        ...
        priors, args.replace, types, args.network_type, args.dynamic, edge_add, 
        edge_remove
    )
    
    results_dir = f"results_snellius/{args.mode}/{args.replace}/"
