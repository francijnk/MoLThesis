...
def smooth_posterior(posterior, prior, alpha):
    # posterior and prior are log-space
    post_probs = np.exp(posterior - np.max(posterior))
    post_probs /= np.sum(post_probs)

    prior_probs = np.exp(prior - np.max(prior))
    prior_probs /= np.sum(prior_probs)

    # combination
    smoothed = (1 - alpha) * post_probs + alpha * prior_probs
...
def generate_network(network_type, pop_size):
    ...
    if network_type == "fully-connected":
    ...
    elif network_type == "small-world" or network_type == "dynamic":
        ...
        raise Exception("Network type should be fully-connected, hierarchical, small-world, scale-free, dynamic, or mobile-phone.")
...
def pick_based_on_communicative_success(population, languages, mode, meanings, signals, error_probability):
    ...
    for prob in com_suc_probs:
        com_suc_probs_reverse.append(m - prob + 1)
...
def create_agent(id, gr_strings, neighbors):
    strength = random.random() # random compressibility strength value between 0 and 1
    priors = [log_non_normed_prior(g, strength) for g in gr_strings]
    priors = normalize_logprobs(priors)

    return Agent(id=id, prior=priors, neighbors=neighbors, age=0)          


def simulation(generations, languages, meanings, signals, rounds, popsize, turnover_rounds, 
               run, error_probability, mode, gr_strings, replace, types,
               network_type, edge_add, edge_remove, alpha):
    results = []
    population_dict = {}
    ...
    for i in range(popsize):
        ...
        agent = create_agent(i, gr_strings, neighbors=neighbors)
        ...
        if network_type == "dynamic":
            ...
            if turnover_rounds > 0:
                for i in range(extra_agent_to_add + 1): # add at least one new agent, and additionally extra if more were removed previously
                    ...
                    new_agent = create_agent(new_id, neighbors = [parent1.id, parent2.id], gr_strings=gr_strings)
                ...
                for agent in population:
                    ...
                    p_add = random.random()
                    if edge_add < p_add:
                        connect_to = create_edge(agent, 0, population_dict, population, network)
                ...
                old_agent = pick_based_on_communicative_success(population, languages, mode, meanings, signals, error_probability)
            ...
            if turnover_rounds > 0:
                for _ in range(replace):    
                    ...
                    new_agent = create_agent(picked_agent.id, gr_strings=gr_strings, neighbors = [n for n in network.neighbors(picked_agent.id)])
...
def main():
    ...
    result, data = simulation(
        ...
        gr_strings, r, types, args.network_type, edge_add, 
        edge_remove, args.alpha
    )

    name = f"h{args.group_round}_v{args.turnover_round}_p{args.pop_size}"
    results_dir = f"results_snellius/{args.mode}/{args.replace}/{args.alpha}/{name}"
    
   
    print("Checkpoint: About to save CSV")
    print("Data preview:", result.head())
